...
export function Topbar({ onReportClick, onMobileMenuClick }: { onReportClick: (id: number) => void; onMobileMenuClick?: () => void }) {
  ...
  return (
    <div className="topbar" role="banner">
      <button className="btn-icon" onClick={onMobileMenuClick} aria-label="메뉴 열기" style={{ display: 'none' }} id="mobile-menu-btn">
        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
          <line x1="3" y1="6" x2="21" y2="6" /><line x1="3" y1="12" x2="21" y2="12" /><line x1="3" y1="18" x2="21" y2="18" />
        </svg>
      </button>

      {breadcrumb && <div className="topbar-breadcrumb" aria-label="현재 페이지">{breadcrumb}</div>}
          ...
          <div className="dropdown-panel" style={{ top: 50, left: 0, right: 0, maxHeight: 320, overflowY: 'auto' }} role="listbox">
            {searchResults.length === 0 ? (
              ...
              searchResults.map((r, i) => (
                <div key={i} className="dropdown-item" role="option" onMouseDown={() => { setSearchOpen(false); onReportClick(r.report_id) }}>
                  <div>
      ...
      <div className="topbar-actions">
        <div ref={notifRef} style={{ position: 'relative' }}>
          <button className="btn-icon badge-wrap" onClick={() => setNotifOpen((o) => !o)} aria-label={`알림${unreadCount > 0 ? ` (${unreadCount}개 읽지 않음)` : ''}`} aria-expanded={notifOpen}>
            ...
            <div className="dropdown-panel" style={{ right: 0, top: 44, width: 360, maxHeight: 480, overflowY: 'auto' }} role="menu">
              ...
              {notifications.length === 0 ? (
                ...
                notifications.slice(0, 20).map((n) => (
                  <div key={n.id} className={`dropdown-item ${n.is_read ? '' : 'unread'}`} onClick={() => handleNotifClick(n)} role="menuitem" tabIndex={0} onKeyDown={(e) => e.key === 'Enter' && handleNotifClick(n)}>
        ...
        <div ref={accountRef} style={{ position: 'relative' }}>
          <button className="account-btn" onClick={() => setAccountOpen((o) => !o)} title={user?.name ?? ''} aria-label="계정 메뉴" aria-expanded={accountOpen}>
          ...
          {accountOpen && (
            <div className="dropdown-panel" style={{ right: 0, top: 46, width: 220 }} role="menu">
              ...
              <button className="dropdown-item" style={{ gap: 10, fontSize: 13, color: 'var(--ink-2)' }} role="menuitem" onClick={() => { setAccountOpen(false); setChangePwOpen(true) }}>
                <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden="true">
                  <rect x="3" y="11" width="18" height="11" rx="2" /><path d="M7 11V7a5 5 0 0 1 10 0v4" />
                </svg>
                비밀번호 변경
              </button>
              <button className="dropdown-item" style={{ gap: 10, fontSize: 13, color: 'var(--red)' }} role="menuitem" onClick={handleLogout}>
                <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden="true">
...
function ChangePasswordModal({ onClose }: { onClose: () => void }) {
  ...
  async function submit() {
    if (!current || !next) { toast('모든 항목을 입력해 주세요.', 'error'); return }
    ...
    try {
      ...
      toast(e.response?.data?.detail ?? '변경에 실패했습니다.', 'error')
