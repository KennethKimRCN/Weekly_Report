...
export default function Members() {
  ...
  const [memberPrimary, setMemberPrimary]   = useState<Map<number, number>>(new Map())
  ...
  function openMemberEditor(team: Team) {
    ...
    team.members.forEach((m) => {
      roles.set(m.user_id, m.role || 'member')
      primary.set(m.user_id, m.primary_team ?? 0)
    })
    setMemberRoles(roles)
    setMemberPrimary(primary)
    setMemberSearch('')
  }
  ...
  function toggleMember(userId: number) {
    setMemberRoles((prev) => {
      ...
      if (next.has(userId)) {
        next.delete(userId)
        setMemberPrimary((p) => { const np = new Map(p); np.delete(userId); return np })
      } else {
        next.set(userId, 'member')
        setMemberPrimary((p) => { const np = new Map(p); np.set(userId, 1); return np })
      }
      return next
    })
  }
  ...
  async function saveTeamMembers() {
    ...
    try {
      const members = Array.from(memberRoles.entries()).map(([user_id, role]) => ({
        user_id,
        role,
        primary_team: memberPrimary.get(user_id) ?? 0,
      }))
      await teamsApi.updateMembers(memberTeam.id, members)
    ...
    <div>
          ...
          <div className="table-wrap" style={{ border: 'none', borderRadius: 'var(--radius)' }}>
            <table>
              ...
              <tbody>
                {users.map((u) => {
                  const primaryTeam =
                    allTeams.find((t) => t.members.some((m) => m.user_id === u.id && m.primary_team)) ??
                    allTeams.find((t) => t.members.some((m) => m.user_id === u.id))
      ...
      {memberTeam && (() => {
        ...
        const sorted = [...users].sort((a, b) => {
          const aIn = memberRoles.has(a.id) ? 0 : 1
          const bIn = memberRoles.has(b.id) ? 0 : 1
          return aIn - bIn || a.name.localeCompare(b.name)
        })
        const visible = q ? sorted.filter((u) => u.name.toLowerCase().includes(q) || u.rank_name?.toLowerCase().includes(q)) : sorted
        const inTeamCount = memberRoles.size

        const ROLE_STYLES: Record<string, React.CSSProperties> = {
          lead:     { background: '#ebf4ff', color: '#2b6cb0', borderColor: '#bee3f8', fontWeight: 600 },
          member:   { background: 'var(--surface-3)', color: 'var(--ink-3)', borderColor: 'var(--border)' },
          observer: { background: 'var(--amber-bg)', color: '#7a5100', borderColor: 'rgba(214,161,0,.25)' },
        }

        return (
          <Modal title={`${memberTeam.name} · 팀원 편집`} onClose={() => setMemberTeam(null)} size="lg"
            footer={
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%' }}>
                <div style={{ fontSize: 12, color: 'var(--ink-4)', display: 'flex', alignItems: 'center', gap: 8 }}>
                  <span>선택된 팀원 <strong style={{ color: 'var(--ink-2)' }}>{inTeamCount}명</strong></span>
                  {leadCount > 1 && (
                    <span style={{ color: 'var(--red)', background: 'var(--red-bg)', border: '1px solid rgba(197,59,50,.2)', borderRadius: 20, padding: '2px 8px', fontSize: 11 }}>
                      ⚠ 리더 {leadCount}명
                    </span>
                  )}
                  {leadCount === 0 && inTeamCount > 0 && (
                    <span style={{ color: '#7a5100', background: 'var(--amber-bg)', border: '1px solid rgba(214,161,0,.25)', borderRadius: 20, padding: '2px 8px', fontSize: 11 }}>
                      리더 미지정
                    </span>
                  )}
                </div>
            ...
            <div style={{ position: 'relative', marginBottom: 14 }}>
              <svg style={{ position: 'absolute', left: 10, top: '50%', transform: 'translateY(-50%)', color: 'var(--ink-5)', pointerEvents: 'none' }}
            ...
            {/* Section header: current members */}
            {inTeamCount > 0 && !q && (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 3, maxHeight: 400, overflowY: 'auto', marginBottom: 2 }}>

              {/* Divider between in-team and out */}
              {(() => {
                let shownDivider = false
                return visible.map((u) => {
                  const inTeam = memberRoles.has(u.id)
                  const role   = memberRoles.get(u.id) ?? 'member'
                  const showDivider = !q && !inTeam && !shownDivider && inTeamCount > 0
                    ...
                    <React.Fragment key={u.id}>
                      {showDivider && (
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8, margin: '6px 0 4px' }}>
                          ...
                          <span style={{ fontSize: 11, fontWeight: 600, color: 'var(--ink-5)', textTransform: 'uppercase', letterSpacing: '0.06em', whiteSpace: 'nowrap' }}>
                            추가 가능한 사용자
                          </span>
                          <div style={{ flex: 1, height: 1, background: 'var(--border)' }} />
                        </div>
                      ...
                      <div
                        ...
                        style={{
                          ...
                          background: inTeam ? 'var(--blue-light)' : 'var(--surface)',
                          border: `1px solid ${inTeam ? 'var(--blue-mid)' : 'var(--border)'}`,
                          cursor: 'pointer',
                      ...
                      >
                        ...
                        <div style={{
                          ...
                          border: `2px solid ${inTeam ? 'var(--blue)' : 'var(--border)'}`,
                          background: inTeam ? 'var(--blue)' : 'transparent',
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          ...
                          <div style={{ fontSize: 11, color: 'var(--ink-4)', marginTop: 1 }}>{u.rank_name}</div>
                        </div>

                        {/* Role selector — only shown when in team */}
                        {inTeam && (
                          <div
                            onClick={(e) => e.stopPropagation()}
                            style={{ display: 'flex', gap: 4, flexShrink: 0 }}
                          >
                            {(['lead', 'member', 'observer'] as const).map((r) => (
                              <button
                                key={r}
                                onClick={(e) => { e.stopPropagation(); setRole(u.id, r) }}
                                style={{
                                  ...
                                  ...(role === r
                                    ? ROLE_STYLES[r]
                                    : { background: 'transparent', color: 'var(--ink-5)', borderColor: 'transparent' }),
                                }}
                              >
                                {r === 'lead' ? '파트리더' : r === 'member' ? '팀원' : '옵저버'}
                              </button>
                            ))}
                          </div>
                        )}
              ...
              {visible.length === 0 && (
                <div style={{ textAlign: 'center', padding: '32px 0', color: 'var(--ink-5)', fontSize: 13 }}>
                  검색 결과가 없습니다.
                </div>
              )}
            </div>
